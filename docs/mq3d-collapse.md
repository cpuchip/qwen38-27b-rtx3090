...
n=6 (01:01Z): bf16, stock FlashAttention, `start_qwen.sh CTX=fast`, no fork KV tier, pool pinned to 20,264 tokens, no pad, 10k-token side-load: died at the second site (the connector scheduler's per-group assert) 15 requests in. The KV tier is out of it entirely; nothing this repository ships is in the path. A speculation-off control could not be obtained from this launcher on the production image: with `SPEC=off`, prefix caching on and Mamba align mode on, the engine reported a 0.0% prefix-cache hit rate for the whole run (246 samples), so every turn re-prefilled and the connector never had a block to reload (two arms, 0 loads each; the runs that completed were clean, 14/18). That 0% is itself a #52-shaped symptom (the snapshot the Mamba group needs is not retained), and it means the drafter's part in the crash is untested here; upstream's own rows put the trigger on the connector returning tokens, with the drafter present in every corrupting cell.

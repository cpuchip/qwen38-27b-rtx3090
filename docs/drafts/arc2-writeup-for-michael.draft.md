...
| Reproduction mode (DFLASH_TOKENS=15) | on this cohort, no gain in accepted tokens; positions 7 to 14 give 1.2 to 1.6 percent of accepted tokens; step cost 2.4x on this card on every boot measured, 0.3 percent on the native 3090; with async on and every step padded to 15 it is 3.7x here and 1.05x there | named: the fork's split-KV verify attention at configured width 15 (whether its query maximum of 16 is the operative part is one cell away). With it off (SPEC_ATTN=0) width 15 runs at 23.6 ms per step on the 4090 with the acceptance intact; on the 3090 the kernel costs nothing either way. Separately, one default-width boot in nine here landed at 45 ms per step instead of 23 (and two of two sampled boots under the forced first kernel config, none natively); the GDN autotune race does not run at the default with the compile cache warm, so the side is decided under sampling on the first request (greedy boots are always fast); the cells that name it are running |
...
Instruments were corrected four times: the launcher inside the image is the fork's (LOOKUP is the alias it honours); the engine's own non-default-args line is the resolved configuration; a rule that two engines on this host slow each other was written from one boot pair and withdrawn when the repeat landed on the other side with the neighbour idle; and the step cost on this card has two separate stories: the width-15 cost is one named kernel (the split-KV verify attention, off means 23.6 ms), and the default-width scatter (one boot in nine at 45 ms instead of 23, clocks and graphs identical, sampled requests only, never on the native card; the forced first kernel config is slow on two sampled boots of two) is decided on the first sampled request by something a greedy request skips, with the GDN autotune race excluded at the default because a warm compile cache never re-races it. Issue 75 gets both halves with their prices; the cells that name the second half are running. Journal lessons 49 to 75.

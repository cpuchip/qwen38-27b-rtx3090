...
| Reproduction mode (DFLASH_TOKENS=15) | on this cohort, no gain in accepted tokens; positions 7 to 14 give 1.2 to 1.6 percent of accepted tokens; step cost 2.4x on this card on every boot measured, 0.3 percent on the native 3090; with async on and every step padded to 15 it is 3.7x here and 1.05x there | named: the fork's split-KV verify attention at configured width 15 (whether its query maximum of 16 is the operative part is one cell away). With it off (SPEC_ATTN=0) width 15 runs at 23.6 ms per step on the 4090 with the acceptance intact; on the 3090 the kernel costs nothing either way. Separately, default-width boots here land at 23 or 45 ms by boot (four slow of ten) and never natively; the GDN autotune race does not run at the default with the compile cache warm, so the side is decided under sampling on the first request (greedy boots are always fast); the cells that name it are running |

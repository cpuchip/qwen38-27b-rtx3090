...
| Reproduction mode (DFLASH_TOKENS=15) | on this cohort, no gain in accepted tokens; positions 7 to 14 give 1.2 to 1.6 percent of accepted tokens; step cost 2.4x on this card on every boot measured, 0.3 percent on the native 3090; with async on and every step padded to 15 it is 3.7x here and 1.05x there | named: the fork's split-KV verify attention at configured width 15 (whether its query maximum of 16 is the operative part is one cell away). With it off (SPEC_ATTN=0) width 15 runs at 23.6 ms per step on the 4090 with the acceptance intact; on the 3090 the kernel costs nothing either way. Separately, the GDN autotune race lands slow on some default-width boots here (23 or 45 by boot) and never natively; winner-printed boots running |
...
Instruments were corrected four times: the launcher inside the image is the fork's (LOOKUP is the alias it honours); the engine's own non-default-args line is the resolved configuration; a rule that two engines on this host slow each other was written from one boot pair and withdrawn when the repeat landed on the other side with the neighbour idle; and the step cost on this card is the Triton autotuner's draw: bimodal per boot at the default (23 or 45 ms, four slow of ten, clocks and graphs identical), and always slow at width 15 (56.5), while under a fixed kernel configuration width 15 costs 4 ms more than width 7. That is issue 75 with a mechanism and a price, measured at the configuration users run; four more default boots with the autotune winners printed are running to name the kernel, and two other candidates (the verify kernel's query maximum, the split-KV verify attention itself) run beside them. Journal lessons 49 to 73.

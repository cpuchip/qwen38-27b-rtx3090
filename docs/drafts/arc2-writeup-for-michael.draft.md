...
The launcher default replays across boots on this host to the token (five boots, one trajectory). Reproduction mode does not: five width-15 boots form exactly two trajectories, differing on the same ten rows, on both cards here, and on the native 3090 only with prefix caching off. The Triton autotune race is excluded as the source (skipping it makes boots diverge more) and so is prefix caching (turning it off does the same); whatever the binary boot state is, its footprint grows with how much a boot computes from scratch, and it never flips a greedy argmax. Issue 75 narrows to the width-15 path.

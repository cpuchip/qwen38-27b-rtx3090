...
| Reproduction mode (DFLASH_TOKENS=15) | on this cohort, no gain in accepted tokens; positions 7 to 14 give 1.2 to 1.6 percent of accepted tokens; step cost 2.4x on this card on every boot measured, 0.3 percent on the native 3090; with async on and every step padded to 15 it is 3.7x here and 1.05x there | the cost is set by the configured width, not the positions used (verifying 7 of 15 costs the same as verifying 15: 56.5 against 57.8 ms), on the 4090 and not on the 3090; a buffer or kernel variant sized from num_speculative_tokens at setup, the fork's split-KV verify attention the named instance; cause cells running |

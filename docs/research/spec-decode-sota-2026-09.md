...
## Addendum from the late sub-agent drains (2026-09-05, all URLs accessed 2026-09-04 by the agents)

- **vLLM 0.29.0 is at release candidate 4.** Tags v0.29.0rc1 through rc4 exist (2026-09-02 to 2026-09-04) as git tags only, with no GitHub Release object and no PyPI wheel, which is why the releases page and PyPI look empty; the tags API (`/repos/vllm-project/vllm/tags`) is the tiebreaker. v0.28.1rc0 exists likewise (2026-08-27). rc4 sha `d2906cc1958658f296aeee8b248deea684f56add`. No milestone, no release branch, no maintainer-stated date; the last two cycles ran two RCs then stable, and this one is at four RCs in three days. Stable "within days" is the agent's read and is UNVERIFIED. main is 858 commits ahead of v0.28.0.
- **The Triton autotune lever that exists is `VLLM_TRITON_FORCE_FIRST_CONFIG`** (default off; monkey-patches `Autotuner.run` to take the first valid config, so it removes the race by skipping it, not by caching a result). Present in the 0.28.0 image at `vllm/envs.py:124` and `vllm/triton_utils/force_first_config.py`, checked in the built tree. There is no Triton autotune cache in vLLM; the only persistent autotune cache is FlashInfer's. The request for a `VLLM_TRITON_AUTOTUNE` switch (vllm-project/vllm#33279) was closed as not planned, so that variable does not exist despite appearing in secondary write-ups. A GDN-specific autotuner OOM on non-SM90 cards (#36598, fixed by #43047: the profiler never ran the GDN Triton kernels, so the autotuner's 16-config benchmark ran after KV cache had claimed the memory) is the same family as #55341 on main, "warm up kernels before capturing CUDA graphs".
- **Batch invariance does not reach the GDN path in 0.28.0**: `batch_invariant.py` registers matmul, softmax, mean, bmm and RMSNorm only; twelve linear-attention and Mamba terms appear zero times in it, and the selector aborts at startup on a GDN backend. PR #45819 (open, updated 2026-09-04) adds GDN invariance by per-sequence loops, which will trade throughput for invariance if it lands as written. Not in any 0.29 RC.
- **EAGLE 3.1 (May 2026) is the end of the EAGLE line, not EAGLE-4**, which does not exist: two booleans on the EAGLE-3 recipe (`fc_norm`, `norm_output`) read from the draft config under the unchanged `"method":"eagle3"`, in vLLM since 0.22. No absolute acceptance length published; the headline is per-user throughput at concurrency 1 on GB200. No EAGLE-3 acceptance or tok/s figure exists on an RTX 3090, 4090 or A5000 in any paper, blog or maintainer benchmark the agent could find; the one peer-reviewed consumer-card datapoint in the survey is OPT-Tree on a single 4090 (Llama-2-7B).
- **Post-0.28.0 main, spec-decode PRs the first pass did not list**: #54826 (honour the draft's attention backend on Model Runner V2), #55245 (spec-decode warm-up device selection), #54418 (default CUDA graph sizes memory-safe), #50488 (capture the widest uniform decode batch by default), #53962 (do not pad spec decode up to max_model_len), #55404 (GDN cudagraph-capture metadata without a device sync). No rejection-sampler change on main since the tag.
